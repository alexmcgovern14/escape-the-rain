/**
 * Script to calculate baseline POI frequencies across a variety of UK locations
 * Run with: npx tsx scripts/calculate-poi-baselines.ts
 */
...
async function calculateBaselines() {
  ...
  // Note: We track PRESENCE (does location have this POI type?), not counts
  // Baseline = frequency across locations (e.g., 0.2 = 20% of locations have museums)
  ...
  console.log("POI Type Frequencies (baseline averages):");
  console.log("=" .repeat(70));
  console.log("Format: POI Type | Frequency (0.0-1.0) | Percentage | Count");
  console.log("=" .repeat(70));
  
  for (const [poiType, presenceCount] of sortedPoiTypes) {
    const frequency = presenceCount / totalLocations;
    baselines[poiType] = frequency;
    console.log(
      `${poiType.padEnd(25)} | ${frequency.toFixed(3).padStart(5)} | ${(frequency * 100).toFixed(1).padStart(5)}% | ${presenceCount.toString().padStart(3)}/${totalLocations}`
    );
  }
...
calculateBaselines().catch(console.error);
